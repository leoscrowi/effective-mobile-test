...
func ValidateDto(dto EditSubscriptionRequset) error {
	...
	if !isValidDateFormatEdit(dto.StartDate) {
		return fmt.Errorf("start_date: invalid format, expected MM-YYYY")
	}

	if !isValidDateFormatEdit(dto.EndDate) {
		return fmt.Errorf("end_date: invalid format, expected MM-YYYY")
	}

	if !isEndDateAfterStartDateEdit(dto.StartDate, dto.EndDate) {
...
func isValidDateFormatEdit(date string) bool {
...
func isEndDateAfterStartDateEdit(startDate, endDate string) bool {
